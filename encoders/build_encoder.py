...
from sam2rad.models.sam.modeling.rd_vit import RDViTEncoder, load_sam_weights_to_rgbcrim_encoder_final_superfixed
from sam2rad.models.sam.modeling.nodem import NOdemEncoder
...
class NOdemImageEncoder(ImageEncoder):
    def __init__(self, net: NOdemEncoder):
        super().__init__()
        self.net = net
        self.img_size = net.img_size

    def freeze_pretrained_parameters(self):
        for name, param in self.net.named_parameters():
            if "feature_adapter" in name or "cross_branch_attention" in name or "Adapter" in name or "MDTAdapter" in name or "FeatureAdapter" in name:
    ...
    def load_checkpoint(self, checkpoint_path: str) -> None:
        try:
            load_sam_weights_to_rgbcrim_encoder_final_superfixed(self.net, checkpoint_path)
            logger.info("NOdem weights loaded successfully from %s", checkpoint_path)
        except Exception as e:
            logger.error("Failed to load NOdem weights: %s", str(e))
            raise e

    def forward(self, *args, **kwargs):
        return self.net(*args, **kwargs)

@register_image_encoder("nodem")
class NOdemEncoderFactory(ImageEncoderFactory):
    def build(self, args) -> NOdemImageEncoder:
        in_chans = args.get("in_channels", 3)
        net = NOdemEncoder(
        ...
        encoder = NOdemImageEncoder(net)

        if "sam_checkpoint" in args:
            encoder.load_checkpoint(args.sam_checkpoint)

        # ✅ 重要：在加载完权重后再冻结
        encoder.freeze_pretrained_parameters()

        return encoder

class RDViTImageEncoder(ImageEncoder):
    def __init__(self, net: RDViTEncoder):
        super().__init__()
        self.net = net
        self.img_size = net.img_size
    ...
    def load_checkpoint(self, checkpoint_path: str) -> None:
        try:
            load_sam_weights_to_rgbcrim_encoder_final_superfixed(self.net, checkpoint_path)
            logger.info("RDViT weights loaded successfully from %s", checkpoint_path)
        except Exception as e:
            logger.error("Failed to load RDViT weights: %s", str(e))
            raise e

    def forward(self, *args, **kwargs):
        return self.net(*args, **kwargs)

@register_image_encoder("rd_vit")
class RDViTEncoderFactory(ImageEncoderFactory):
    def build(self, args) -> RDViTImageEncoder:
        in_chans = args.get("in_channels", 3)
        net = RDViTEncoder(
            img_size=args.image_size,
            patch_size=16,
...
class SAMViTBFactory(SAMImageEncoderFactory):
    ...
    def get_backbone(self, args) -> nn.Module:
        print(f"[RDViTFactory] args.embed_dim = {args.embed_dim}")
        print(f"[RDViTFactory] args.image_size = {args.image_size}")
        print(f"[RDViTFactory] args.patch_size = {args.patch_size}")
        print(f"[RDViTFactory] args.prompt_embed_dim = {args.prompt_embed_dim}")
        return ViTBackbone(
            image_size=args.image_size,
            patch_size=args.patch_size,
            prompt_embed_dim=args.prompt_embed_dim,
