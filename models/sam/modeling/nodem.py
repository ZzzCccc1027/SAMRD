...
class NOdemEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor = None):
        # 1) 做 patch embedding：BCHW -> BHWC
        x = self.patch_embed(x)  # [B, H_p, W_p, C]

        # 2) 加绝对位置编码（会在 batch 维度自动广播）
        if self.pos_embed is not None:
            # 如果实际 H_p/W_p 与 pos_embed 不一致，可在此插值（可选，见下方“可选插值”）
            x = x + self.pos_embed  # [1, H_p, W_p, C] 与 [B, H_p, W_p, C] 广播相加

        # 3) 通过每个 block
        high_res_features = []
        for blk in self.blocks:
            x = blk(x)  # 仍然是 [B, H_p, W_p, C]
            high_res_features.append(x.permute(0, 3, 1, 2))  # 存成 [B, C, H_p, W_p]

        # 4) Neck 输出最终特征图
        final_out = self.neck(x.permute(0, 3, 1, 2))  # [B, out_chans, H_p, W_p]
...
def test_load_sam_to_rdvit(
    ...
    # 1. 创建随机输入
    B, H, W, C = 4, img_size, img_size, in_chans
    rgb_input = torch.randn(B, C, H, W)

    # 2. 初始化RDViTEncoder
    encoder = NOdemEncoder(
        img_size=img_size,
    ...
    encoder.eval()
    with torch.no_grad():
        final_out, high_res_features = encoder(rgb_input)
